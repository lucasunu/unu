...
from config import WAITING_TIME, DEFAULT_GAMEMODE, MIN_PLAYERS, SUDOS, TOKEN
...
@user_locale
def buck_sudp(bot, update):
	chat = update.message.chat_id
	if str(update.message.from_user.id) == '652890017':
		ctime = int(time.time())
		bot = amanobot.Bot(TOKEN)
		sent = bot.sendMessage(chat,'Iniciando....')
		...
		bot.editMessageText((chat, sent['message_id']), 'enviando...')
		bot.sendDocument(chat, open('backup-{}.zip'.format(ctime), 'rb'))
		bot.editMessageText((chat, sent['message_id']), '✅ Backup concluído!')
		os.remove('backup-{}.zip'.format(ctime))

@user_locale
def restart_sudo(bot, update):
	chat = update.message.chat
	if update.message.from_user.id in SUDOS:
		send_async(bot, chat.id,
			text=_('reiniciando....'),
			reply_to_message_id=update.message.message_id)
...
def process_result(bot, update, job_queue):
    ...
    if result_id in ('hand', 'gameinfo', 'nogame'):
        ...
        game.turn()
    elif 'udraw_' in result_id:
    	player = gm.userid_current[int(result_id[5:])]
    	player.draw()
...
dispatcher.add_handler(CommandHandler('sudo', sudos))
dispatcher.add_handler(CommandHandler('backup', buck_sudp))
dispatcher.add_handler(CommandHandler('kill', kill_game))
dispatcher.add_handler(CommandHandler('join', join_game))
dispatcher.add_handler(CommandHandler('leave', leave_game))
dispatcher.add_handler(CommandHandler('exec', exec_sudo))
dispatcher.add_handler(CommandHandler('kick', kick_player))
dispatcher.add_handler(CommandHandler('open', open_game))
dispatcher.add_handler(CommandHandler('close', close_game))
dispatcher.add_handler(CommandHandler('restart', restart_sudo))
